...
2. Значения `let`-переменных вычисляются на уровне `describe`.
    ...
    `Proc`-форму или обычный `let(…) { … }` внутри контекста.

2. Контекст можно исключить из обработки, приписав к началу `x`:
...
3. Можно определить свой метод форматирования строки для отчёта:
