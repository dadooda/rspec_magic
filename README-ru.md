...
«Отвечает» тот контекст, формат текста которого допускает преобразование в имя метода.
См. (1) — контекст `"intermediate context"` является самым близким по вложенности,
однако `m` вычислилась как `:some_action`.
