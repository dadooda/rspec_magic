...
а потом через `include_context "то сё"` включаем материал в текущий тест.
...
Казалось бы — вот оно счастьюшко, сочиняй свои расширения, разноси по общим контекстам-«библиотекам»,
где надо импортируй и радуйся.
...
На деле приходится придумывать общим контекстам уникальные имена, и *в каждом* spec-файле
перечислять импортируемое унылым повторяющимся списком:
