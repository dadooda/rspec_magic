...
1. `context_when` эффективно работает в паре с [use_letset](#use_letset),
    обычно для задания атрибутов тестируемого объекта.

2. Значения `let`-переменных вычисляются на уровне `describe`.
    Если нужны значения, вычисляемые на уровне `it`, следует использовать 
    обычный `let(…) { … }` внутри контекста или `Proc`-форму:

    ```rb
    context_when x: -> { … }, x_label: "…" do
    end
    ```

3. Контекст можно исключить из обработки, приписав к началу `x`:
...
4. Можно задать свой метод форматирования строки для отчёта:
